...
if __name__ == '__main__':
    ...
    for f in sys.argv[1:]:
        with open(f, 'rb') as fd:
            data = fd.read()
            if data[0].to_bytes(1,'big') == b'\x76':  # process .kirbi
                # rem dump
                etype = str(decoder.decode(data)[0][2][0][3][0])
                if etype != "23":
                    sys.stderr.write("Unsupported etype %s seen! Please report this to us.\n" % etype)
                et = decoder.decode(data)[0][2][0][3][2]
                sys.stdout.write("$krb5tgs$unknown:$krb5tgs$%s$" % etype + bytes(et[:16]).hex() + "$" + bytes(et[16:]).hex() + "\n")
                ...
                for ticket in data.strip().split('\n'):
                    ...
                    et = decoder.decode(ticket.decode('hex'))[0][4][3][2]
                    sys.stdout.write("$krb5tgs$unknown:$krb5tgs$%s$" % etype + bytes(et[:16]).hex() + "$" + bytes(et[16:]).hex() + "\n")
